...
// node with branches instead uses `attribute` as a question: GKDecisionTree.findAction looks that
// key up in the answers map it's given, and descends into whichever branch matches the answer.
//
...
// querying (GKDecisionTree.findAction) — but a Kotlin developer has no other way to introspect or
// debug a tree they've built, so it's exposed here (also used by GKDecisionTree.prettyPrint).
...
public class GKDecisionNode internal constructor(
    ...
    // attribute. Takes a Kotlin lambda rather than GameplayKit's NSPredicate, per this library's
    // established design principle (see GKRule).
    public fun createBranch(
    ...
    // other weighted branches, using the owning GKDecisionTree's randomSource.
    public fun createBranch(
        weight: Int,
        attribute: Any,
