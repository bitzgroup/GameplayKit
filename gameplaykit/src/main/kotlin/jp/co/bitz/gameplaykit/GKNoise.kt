...
// mirroring GameplayKit's GKNoise. Each operation below (add, multiply, displace, ...) mutates
// this instance in place by wrapping its current sampling function, the same "builder via
...
public class GKNoise(noiseSource: GKNoiseSource) {
    ...
    // Samples the noise field on the X/Z plane (Y = 0) — the conventional ground plane for
    // terrain-style 2D use, mirroring GameplayKit's `value(at:)`.
